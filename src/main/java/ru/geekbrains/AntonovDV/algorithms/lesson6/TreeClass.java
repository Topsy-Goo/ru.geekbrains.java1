...
// (Вспомогательная.) Скатываемся по ПРАВОМУ склону ветки до конца.
    private Node goRDeep (Node node)
    {
        while (node != null && node.rightHand != null)
            node = node.rightHand;
...
// Итератор, сортирующий элементы по убыванию.
    public Iterator<E> reversedIterator ()
    {
        return new Iterator<E> ()
        {
            Node walker = goRDeep (root);
            ...
            @Override public E next ()
            {
                ...
                // тут всё то же самое, что и в прямом итераторе, но лево изменено на право.
                while (walker != null)
                {
                    if (walker.rightHand != null)
                    {
                        if (tmp == walker.rightHand) // 1а
                            break;
                        if (tmp == walker.leftHand) // 2а
                        {
                            tmp = walker;
                            walker = walker.parent;
                        }
                        else if (tmp == walker) // 1б
                        {
                            if (walker.leftHand != null)
                            {
                                walker = goRDeep (walker.leftHand);
                                break;
                            }
                            else walker = walker.parent;
                        }
                        //else throw new RuntimeException ("ERROR (1) @ reversedIterator.next().");
                    }
                    else if (walker.leftHand == null) // без рук
                    {
                        walker = walker.parent;
                    }
                    else if (tmp == walker) // (1)
                    ...
                    else if (tmp == walker.leftHand) // (2)
                    {
                        tmp = walker;
                        walker = walker.parent;
                    }
                    //else throw new RuntimeException ("ERROR (2) @ reversedIterator.next().");
                }//while
...
//------------------------------- Остальные
    ...
    @Override public void display ()
    {
        ...
        Iterator<E> it = reversedIterator ();
        int i = 0;
        while (it.hasNext ())
        {
            sb.append (it.next ());
